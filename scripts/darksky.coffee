...
#   hubot is it raining - Get the rain forecast for HUBOT_DARK_SKY_DEFAULT_LOCATION
#   hubot is it raining in <location> - Get the rain forecast for <location>
#
# Notes:
    ...
    msg.http(googleurl)
      ...
      .get() (err, res, body) ->
        ...
        if result.results.length > 0
          lat = result.results[0].geometry.location.lat
          lng = result.results[0].geometry.location.lng
          darkSkyRain msg, lat,lng , (darkSkyText) ->
            response = "Weather for #{result.results[0].formatted_address}. #{darkSkyText}"
            msg.send response
        else
          msg.send "Couldn't find #{location}"
...
darkSkyRain = (msg, lat, lng, cb) ->
  ...
  msg.http(url)
    .get() (err, res, body) ->
      ...
      response = "Currently: #{result.minutely.summary}"
      cb response