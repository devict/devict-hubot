...
module.exports = function(robot) {
  robot.respond(/events/i, function(msg) {
    var url = "http://api.meetup.com/2/events?status=upcoming&order=time&limited_events=False&group_urlname=devict&desc=false&offset=0&photo-host=public&format=json&page=20&fields=&sig_id=73273692&sig=9cdd3af6b5a26eb664fe5abab6e5cf7bfaaf090e";
    ...
    msg.http(url).get()(function(err, res, body) {
      ...
      response.results.forEach(function(event) {
        eventStr += (new Date(event.time)) + " :: " + event.name + " @ " + event.venue.name + "\n";
      });
